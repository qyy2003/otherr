...
struct InStream
{
    ...
    void jsonSafeWrite(std::FILE * file, const char* msg);
    ...
    if (resultName != "")
    {
        ...
        if (appesMode)
        {
                ...
                if (res == -1)
                {
                    ...
                    std::fprintf(resultFile, "\"Ratio\": %s, ", stringPoints.c_str());
                }
                else if (res == 0 || res == 2) std::fprintf(resultFile, "\"Ratio\": 1, ");
                else if (res == 1) std::fprintf(resultFile, "\"Ratio\": 0, ");
                else std::fprintf(resultFile, "\"Ratio\": -1, ");
                std::fprintf(resultFile, "\"Details\": \"");
                jsonSafeWrite(resultFile, msg);
                std::fprintf(resultFile, "\"}\n");
            #else
...
// ...
void InStream::jsonSafeWrite(std::FILE * file, const char* msg)
{
    size_t lmsg = strlen(msg); int dots = 0;
    if (lmsg > 64) lmsg = 60, dots = 1;
    for (size_t i = 0; i < lmsg; i++)
    {
        if (msg[i] == '"')
        {
